...
export class NexusAgentRuntime {
  public async executePlan(
    ...
    for (let i = 0; i < run.plan.steps.length; i++) {
      ...
      const depsSatisfied = step.dependencies.every(depId => {
        const parent = run.plan.steps.find(s => {
          if (s.id === depId) return true;
          const numMatch = depId.match(/(\d+)$/);
          return numMatch && s.stepNumber === parseInt(numMatch[1], 10);
        });
  ...
  private resolveStepParameters(
    ...
    for (const depId of step.dependencies) {
      ...
      // Feed analysis summary and folder from previous steps to create_drive_file
      if (step.tool === 'create_drive_file') {
        if (depData.summaryLines && depData.summaryLines.length > 0 && (!params.content || params.content === 'auto')) {
          params.content = `EXECUTIVE SUMMARY\nGenerated by NEXUS Workspace Autopilot Engine™\nTimestamp: ${new Date().toISOString()}\n\nHighlights:\n${depData.summaryLines.map((l: string) => `• ${l}`).join('\n')}`;
        }
      }
    }

    // Resolve target folder for create_drive_file from search_drive results or context
    if (step.tool === 'create_drive_file' && (!params.folder || params.folder === 'auto')) {
      // Find original file folder from stepResults
      let originalFolder = '';
      for (const res of Object.values(stepResults)) {
        if (res && res.files && res.files[0]?.folder) {
          originalFolder = res.files[0].folder;
          break;
        }
      }
      params.folder = originalFolder || context?.activeTask?.toolResults?.search_drive?.files?.[0]?.folder || '/Finance/Invoices';
    }

    // Resolve content for create_drive_file if still auto
    if (step.tool === 'create_drive_file' && (!params.content || params.content === 'auto')) {
      const summary =
        context?.activeTask?.toolResults?.analyze_document?.summaryLines?.join('\n') ||
        context?.resolvedReferences?.summary?.label ||
        'Summary created and verified by NEXUS Autopilot.';
      params.content = `EXECUTIVE SUMMARY\nGenerated by NEXUS Workspace Autopilot Engine™\n\n${summary}`;
    }
...
- Keep it concise, scannable, and clean.`;
  ...
  private synthesizeDeterministicResponse(run: ExecutionRun, personality?: SystemPersonality): string {
    ...
    // 8. File Creation (including Summarize & Save Alongside)
    if (results.create_drive_file) {
      const cr = results.create_drive_file;
      const originalFile = results.search_drive?.files?.[0]?.name || 'the original document';
      const analysis = results.analyze_document;
      let highlights = '';
      if (analysis?.summaryLines?.length > 0) {
        highlights = `\n\nExecutive Highlights:\n${analysis.summaryLines.map((l: string) => `• ${l}`).join('\n')}`;
      }
      return `Invoice located, summarized, and saved successfully.\n\nSummary saved as "${cr.name}" in folder "${cr.folder}" alongside ${originalFile}.${highlights}\n\n✓ Document verified and stored`;
    }

    // 9. Document Analysis / Summary
